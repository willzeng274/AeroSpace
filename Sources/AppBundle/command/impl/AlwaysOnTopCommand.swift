...
struct AlwaysOnTopCommand: Command {
    ...
    func run(_ env: CmdEnv, _ io: CmdIo) async -> BinaryExitCode {
        ...
        guard let rect = try? await macWindow.getAxRect(.cancellable) else {
            return .fail(io.err("Failed to get window frame"))
        }
        ...
        guard await OverlayManager.shared.pin(windowId: window.windowId, frame: frame, macWindow: macWindow) else {
            return .fail(io.err("Failed to create overlay. Screen Recording permission may be required."))
        }
        return .succ
    }
}
