...
(back|forward)::
Navigate focus history, similarly to Vim's Ctrl-O (back) and Ctrl-I (forward).
`focus back` moves to the previously focused window, while `focus forward` moves forward after navigating back.
Focusing a new window from the middle of history discards the forward entries, and closed windows are skipped automatically.
This mode is incompatible with all focus flags.
